...
class FileTreeElem():
    ...
    def get_size_of_smallest_dir_to_delete(self, space_to_free: int):
        if self.sum_size >= space_to_free and not self.is_file:
            child_values = []
            for child in self.childs:
                value = child.get_size_of_smallest_dir_to_delete(space_to_free)
                if value is not None:
                    child_values.append(value)
            child_values.append(self.sum_size)
...
if req.status_code == 200 and req.headers['content-type'] == "text/plain":
    ...
    free_space = 70000000 - root.get_sum_size()
    needed_space = 30000000
    space_to_free = needed_space - free_space
    print(root.get_size_of_smallest_dir_to_delete(space_to_free))
